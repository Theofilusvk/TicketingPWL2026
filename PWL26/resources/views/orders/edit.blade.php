                ...
                <div class="card-body">
                    <form action="{{ route('orders.update', $order->order_id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('event_id') is-invalid @enderror" id="event_id" name="event_id" required>
                                ...
                                @foreach($events as $event)
                                    <option value="{{ $event->event_id }}" {{ old('event_id', $order->event_id) == $event->event_id ? 'selected' : '' }}>#{{ $event->event_id }}</option>
                                @endforeach
                            </select>
                            @error('event_id')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" step="0.01" class="form-control @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $order->price) }}" required>
                            @error('price')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
