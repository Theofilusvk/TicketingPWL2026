...
<div class="container">
    <div class="row">
        <div class="col-md-8 offset-md-2">
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('order-items.update', $item->order_item_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('order_id') is-invalid @enderror" id="order_id" name="order_id" required>
                                @foreach($orders as $order)
                                    <option value="{{ $order->order_id }}" {{ old('order_id', $item->order_id) == $order->order_id ? 'selected' : '' }}>#{{ $order->order_id }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('category_id') is-invalid @enderror" id="category_id" name="category_id" required>
                                @foreach($categories as $category)
                                    <option value="{{ $category->category_id }}" {{ old('category_id', $item->category_id) == $category->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('ticket_type') is-invalid @enderror" id="ticket_type" name="ticket_type" required>
                                <option value="regular" {{ old('ticket_type', $item->ticket_type) == 'regular' ? 'selected' : '' }}>Regular</option>
                                <option value="vip" {{ old('ticket_type', $item->ticket_type) == 'vip' ? 'selected' : '' }}>VIP</option>
                                <option value="premium" {{ old('ticket_type', $item->ticket_type) == 'premium' ? 'selected' : '' }}>Premium</option>
                            </select>
