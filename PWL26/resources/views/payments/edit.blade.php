                ...
                <div class="card-body">
                    <form action="{{ route('payments.update', $payment->payment_id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('order_id') is-invalid @enderror" id="order_id" name="order_id" required>
                                @foreach($orders as $order)
                                    <option value="{{ $order->order_id }}" {{ old('order_id', $payment->order_id) == $order->order_id ? 'selected' : '' }}>#{{ $order->order_id }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('payment_gateway') is-invalid @enderror" id="payment_gateway" name="payment_gateway" value="{{ old('payment_gateway', $payment->payment_gateway) }}" required>
