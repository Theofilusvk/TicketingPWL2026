...
type StoreContextValue = {
  ...
  getEventStock: (eventId: string | number) => number
  ...
  updateEvent: (id: string | number, updates: Partial<EventData>) => void
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkout = (tickets: Ticket[], earnedCredits: number, cartItemIdsToRemove: string[]) => {
    setStore(prev => {
      ...
      const ticketCountByEvent: Record<string, number | string> = {}
      removedItems.forEach(item => {
        if (item.ticketId && item.eventId) {
          ticketCountByEvent[item.eventId] = (ticketCountByEvent[item.eventId] as number || 0) + 1
        }
      })
      const updatedEvents = prev.events.map(e => {
        const sold = ticketCountByEvent[e.id]
        if (sold) {
          const newLeft = Math.max(0, e.ticketsLeft - (sold as number))
  ...
  const getEventStock = (eventId: string | number): number => {
    const event = store.events.find(e => e.id === eventId || e.id == eventId)
  ...
  const updateEvent = (id: string | number, updates: Partial<EventData>) => setStore(prev => ({
    ...prev,
    events: prev.events.map(e => e.id === id || e.id == id ? { ...e, ...updates } : e)
  }))
  const deleteEvent = (id: string | number) => setStore(prev => ({ ...prev, events: prev.events.filter(e => e.id !== id && e.id != id) }))
  ...
  const reloadEvents = async () => {
    ...
    try {
      ...
      console.warn('Failed to reload events from API:', error)
    } finally {
      setEventsLoading(false)
    }
  }
